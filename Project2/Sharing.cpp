...
void prefillTree(BST theTree, int high){
    ...
    for(long i = 0; i < high; i++) {
        Node * volatile node = new Node(myGen.createRandomNumber());
        theTree.add(node);
    }
}
...
long ranges[] = {16, 256, 4096, 65536, 1048576};
...
#define OPTYP      BST_NO_LOCK                      // set op type
                                                    // 10 : BST lockless
                                                    // 11 : BST RTM 
                                                    // 12 : BST no Lock
...
ALIGN(64) UINT64 cnt3;                                    // NB: in Debug mode allocated in cache line occupied by cnt0
     ...
     while (1) {
	   for(int i = 0; i < NOPS; i++){
		 switch (OPTYP) {
			case BST_NO_LOCK:
			  // create the numbers
			  INT64 val = myGenerator.createRandomNumber();
			  Node  * volatile node = new Node(val);
			  if( addOrRemove() == 0) bstTree.add(node);
		  ...
		  //    	UINT64 val = myGenerator.createRandomNumber();
				//Node  * volatile node = new Node(val);
		  //    	while (_InterlockedExchange_HLEAcquire(&lock, 1))  // wait for transaction to begin.
		  //    		do { _mm_pause(); } while (lock == 1); // abort the transaction
		  //    	if (addOrRemove() == 0) bstTree.add(node);
		  //    	else bstTree.remove(val);
